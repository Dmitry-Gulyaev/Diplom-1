...
<body>
	...
	<div class = "content">
		...
		<p align = "right"> <a href="help.php" target="_blank"><img src="http://www.gifmania.ru/Animated-Gifs-Animirovannykh-Alfavitov/Animations-Punctuation-Marks/Images-Question-mark/Question-mark-26249.gif" style="vertical-align: middle" alt="Справка" height = "20px" title = "Справка" /></a> </p>
		...
		<?
			...
			{
				header ('Location: recruitment.php?act=add_orders');
			}
		?>
			...
			{ 
				...
				else
				{
						...
						else 
						{
							...
							echo "</tr> <tr>";
							echo "<td>Груз: </td><td><textarea placeholder='Опишите груз. Что он из себя представляет, массу, габариты (max 200 символов)' name = 'freight' maxlength = '200' style = 'width:300px; height:250px;resize: none;'></textarea> </td>";
							echo "</tr> <tr>";
							echo "<td>Цена в пределах: </td> <td><input required type = 'number' name = 'sell'> рублей</td>";
							echo "</tr> <tr>";
							echo "<td>Едете ли Вы с грузом?:  </td> <td><label><input required type = 'radio' name = 'going' value = 'Yes' >Да</label>  <label><input required type = 'radio' name = 'going' value = 'No'>Нет</label></td>";
							echo "</tr> <tr>";
					...
					{
						echo "Найдите перевозчика, который больше всего Вам подходит.";
						echo "Если цена не указана, то она значения не имеет.";
						...
						echo 'Откуда: <select  name="SEARCHselect_A" style="width: 100px"> 
								...
								echo 'Куда: <select  name="SEARCHselect_B" style="width: 100px"> 
								...
								echo 'Автомобиль: 	<select name = "car">
					...
					if (isset ($_POST['save']))
					{
						if ($_POST['going'] == "No")
						{
							echo "<p class = 'red'> Вы указали, что не поедете вместе со своим грузом и доверете его перевозчику. </p> <p> В целях безопасности рекомендуем скачать файл - образец договора и заполнить в нем данные о перевозчике. В случае, если перевозчик не доставит груз в указанное место, Вы сможете обратиться в полицию с этими данными. <a href='images/file.docx' download><img src = 'images/download.png' alt = 'Скачать файл' title = 'Скачать файл' style = 'vertical-align: middle'> </a> </p>";
						}	
						echo "<p class = 'green'>Заказ сохранен<p>";
						$ins  = mysql_query ("INSERT INTO `orders` SET `id_user` = '".$_SESSION['id']."', `type` = 'Нужен перевозчик', `sell` = '".$_POST['sell']."', `locality_A` = '".$_POST['select_A']."',`locality_B` = '".$_POST['select_B']."', `going` = '".$_POST['going']."', `contact` = '".$_POST['contact']."', `note` = '".$_POST['freight']."', `car` = '".$_POST['car']."'");
					}
					...
					if (isset ($_POST['search_orders']))
					{
						...
						if ($_POST['max_sell'] == 0)
						{
								$q = "SELECT * FROM `orders` WHERE `type` = 'Нужен заказчик' AND `locality_A` = '".$_POST['SEARCHselect_A']."' AND `locality_B` = '".$_POST['SEARCHselect_B']."' AND`car` = '".$_POST['car']."'";
							...
							else
							{
								...
								if (mysql_affected_rows() > 0)
								{
									...
									do 
									{
										...
										$q = "SELECT * FROM `orders` WHERE `type` = 'Нужен заказчик' AND (`locality_A` = '".$_POST['SEARCHselect_A']."' AND  `locality_B` = '".$punkt_B."')";
												$result = mysql_query($q);
												if (mysql_affected_rows() > 0) 
											{ 
						...
						{
							$q = "SELECT * FROM `orders` WHERE `type` = 'Нужен заказчик' AND `sell` <= '".$_POST['max_sell']."' AND `locality_A` = '".$_POST['SEARCHselect_A']."' AND `locality_B` = '".$_POST['SEARCHselect_B']."' AND`car` = '".$_POST['car']."'";
							...
							else
							{
								...
								if (mysql_affected_rows() > 0)
								{
									...
									do 
									{
										...
										$q = "SELECT * FROM `orders` WHERE `type` = 'Нужен заказчик' AND `sell` <= '".$_POST['max_sell']."' AND (`locality_A` = '".$_POST['SEARCHselect_A']."' AND  `locality_B` = '".$punkt_B."')";
												$result = mysql_query($q);
												if (mysql_affected_rows() > 0) 
											{ 
